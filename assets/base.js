class Slider extends HTMLElement {
    constructor() {
        ...
        inputSliderValue.innerHTML = this.getAttribute('value') + ' ' + this.getAttribute('prefix');
    ...
    connectedCallback() {
        ...
        for (const card of cards) {
            if (inputSlider.id == 'Blur') {
                ...
                card.style.background = 'rgba(255, 255, 255, ' + inputSlider.value + ')';
            } else if (inputSlider.id == 'Borderradius') {
                card.style.borderRadius = inputSlider.value + 'px';
            } else if (inputSlider.id == 'Outline') {
                card.style.border = '1px solid rgba(255, 255, 255, ' + inputSlider.value + ')';
            }
        }
        ...
        inputSlider.addEventListener('input', () => {
            ...
            sliderOutput.innerHTML = inputSlider.value + ' ' + this.getAttribute('prefix');
            ...
            for (const card of cards) {
                if (inputSlider.id == 'Blur') {
                    card.style.backdropFilter = 'blur(' + inputSlider.value + 'px)';
                    elementBlur.innerHTML = "backdrop-filter: blur" + "(" + inputSlider.value + 'px' + ");";

                } else if (inputSlider.id == 'Transparency') {
                    card.style.backgroundColor = 'rgba(' + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + inputSlider.value + ')';
                    elementBackground.innerHTML = "background: rgba" + "(" + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + inputSlider.value + ");";

                } else if (inputSlider.id == 'Borderradius') {
                    card.style.borderRadius = inputSlider.value + 'px';
                    elementBorderradius.innerHTML = "border-radius: " + inputSlider.value + 'px' + ";";

                } else if (inputSlider.id == 'Outline') {
                    card.style.border = '1px solid rgba(' + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + inputSlider.value + ')';
                    elementBorder.innerHTML = "border: 1px solid rgba" + "(" + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + inputSlider.value + ");";
                }
...
boxshadowCheckbox.addEventListener('change', () => {
    for (const card of cards) {
        if (boxshadow.checked == true) {
            ...
            elementBoxshadow.innerText = "box-shadow: 0 8px 32px 0 rgba(55, 55, 55, 0.36);"
        } else {
...
colorPicker.addEventListener('change', () => {
    ...
    for (const card of cards) {
        card.style.backgroundColor = 'rgba(' + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + transparencyValue + ')';
        card.style.border = '1px solid rgba(' + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + transparencyValue + ')';

        elementBackground.innerHTML = "background: rgba" + "(" + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + transparencyValue + ");";
        elementBorder.innerHTML = "border: 1px solid rgba" + "(" + rbaColor.r + ', ' + rbaColor.g + ', ' + rbaColor.b + ', ' + transparencyValue + ");";
    }
});